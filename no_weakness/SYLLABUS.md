...
|---|:---:|:---:|:---:|
| [Python](01_python/00_syllabus.md) | ●○○ | ●●● | ●●● |
| [Concurrency](02_concurrency/00_syllabus.md) | ○○○ | ●●● | ●●● |
| [SQL](03_sql/00_syllabus.md) | ●●● | ●●● | ●●○ |
| [JavaScript](04_javascript/00_syllabus.md) | ○○○ | ●○○ | ●●● |
| [TypeScript](05_typescript/00_syllabus.md) | ○○○ | ●●○ | ●●● |
| [MongoDB](06_mongodb/00_syllabus.md) | ●○○ | ●●● | ●●○ |
| [BigQuery](07_bigquery/00_syllabus.md) | ●●● | ●●● | ○○○ |

**Data analyst.** The interview is `03_sql/04` and `07_bigquery/04` — window functions and analytical SQL at scale — harder than everything else in this repo combined. Add `03_sql/05` because correctness questions follow directly from the DQA supervision claim on the resume, and the `COUNT(col)` trap is the most common silent reporting error there is. Nobody will ask about the method resolution order. Nobody will ask about the GIL.

**Data engineer.** The widest surface, and the one the CV supports best. It runs on pipeline correctness rather than language trivia: `03_sql/01` and `03_sql/03` for planner and isolation, the whole MongoDB-to-BigQuery arc in `06_mongodb/05` and `07_bigquery/05`, `07_bigquery/02` for cost, and `02_concurrency/07` for idempotency and delivery semantics. The single line *"Implemented ETL data pipelines via Apache Beam to migrate MongoDB data to BigQuery"* is the most probable subject of a fifteen-minute deep dive in any interview for this role.
...
1. [`03_sql/04`](03_sql/00_syllabus.md) — window functions, frames, ranking, gaps-and-islands, cohort retention with censoring. **This is the interview.**
2. [`03_sql/05`](03_sql/00_syllabus.md) — NULL semantics, the `COUNT` trio, money types, time zones. This is where correctness questions land.
3. [`03_sql/02`](03_sql/00_syllabus.md) — evaluation order and the `LEFT JOIN` row-loss bug.
4. [`07_bigquery/04`](07_bigquery/00_syllabus.md) — `UNNEST`, `QUALIFY`, approximate aggregates and when approximation is defensible.
5. [`07_bigquery/02`](07_bigquery/00_syllabus.md) — cost control, because an analyst who knows why `SELECT *` is expensive stands out immediately.
6. [`03_sql/01`](03_sql/00_syllabus.md) — indexes and plans, if there is time.

### Data engineer

1. [`03_sql/01`](03_sql/00_syllabus.md) — indexes and the planner.
2. [`03_sql/03`](03_sql/00_syllabus.md) — transactions, isolation, deadlocks.
3. [`06_mongodb/01`](06_mongodb/00_syllabus.md) and [`06_mongodb/02`](06_mongodb/00_syllabus.md) — modelling, then indexes and `explain`.
4. [`07_bigquery/02`](07_bigquery/00_syllabus.md) — partitioning, clustering, cost.
5. [`07_bigquery/03`](07_bigquery/00_syllabus.md) — loading, streaming, idempotency.
6. [`06_mongodb/05`](06_mongodb/00_syllabus.md) → [`07_bigquery/05`](07_bigquery/00_syllabus.md) — the two halves of the flagship bullet, read back to back.
7. [`02_concurrency/07`](02_concurrency/00_syllabus.md) — MVCC, the connection-pool ceiling, delivery semantics.
8. [`01_python/07`](01_python/00_syllabus.md) — lazy pipelines and memory-bounded processing.

### Fullstack SWE

1. [`01_python/01`](01_python/00_syllabus.md) — the object model and descriptors.
2. [`01_python/05`](01_python/00_syllabus.md) — bytecode, then
3. [`02_concurrency/01`](02_concurrency/00_syllabus.md) and [`02`](02_concurrency/00_syllabus.md) — the GIL and races.
4. [`02_concurrency/04`](02_concurrency/00_syllabus.md) — asyncio, including why a blocking call in an `async def` slows unrelated endpoints.
5. [`04_javascript/01`](04_javascript/00_syllabus.md) — closures and the stale-closure bug he has already shipped a fix for.
6. [`04_javascript/03`](04_javascript/00_syllabus.md) — the event loop.
7. [`04_javascript/02`](04_javascript/00_syllabus.md) — prototypes and `this`.
8. [`05_typescript/02`](05_typescript/00_syllabus.md) — generics, inference, variance.
9. [`05_typescript/04`](05_typescript/00_syllabus.md) — typing the boundary, and the Pydantic-versus-Zod comparison.
10. [`01_python/03`](01_python/00_syllabus.md) — decorators and metaprogramming.
...
**Threads for CPU work: JavaScript wins, Python loses.** `worker_threads` are isolated V8 heaps with no shared interpreter state, so there is nothing for a global lock to protect and CPU work genuinely parallelises. Python threads on the same benchmark show no speedup at all. Both runtimes pay the identical price for that isolation — structured clone on one side, pickle on the other. Lives across [`02_concurrency/01`](02_concurrency/00_syllabus.md) and [`06`](02_concurrency/00_syllabus.md).

**Typing runs in opposite directions.** Python keeps annotations at runtime, so Pydantic reads the types and derives validation from them. TypeScript erases them entirely, so Zod does the reverse — you write the validator and infer the type from it. Same destination, opposite direction of travel. Lives across [`01_python/06`](01_python/00_syllabus.md) and [`05_typescript/04`](05_typescript/00_syllabus.md).

**N+1 is one shape in three costumes.** An ORM lazy load, a correlated subquery, and a `$lookup` executing per input document are the same mistake: a cheap operation run once per row. The measured version is worth quoting — one index moved a query from 19,183 ms to 1.1 ms. Lives across [`03_sql/07`](03_sql/00_syllabus.md), [`03_sql/01`](03_sql/00_syllabus.md) and [`06_mongodb/03`](06_mongodb/00_syllabus.md).

**Relational versus document is a question about *when you decide*.** MongoDB makes you decide your query patterns at modelling time and is excellent when you can. PostgreSQL makes you pay upfront in normalisation and buys flexibility later. Neither is the safe default. Lives across [`03_sql/06`](03_sql/00_syllabus.md) and [`06_mongodb/01`](06_mongodb/00_syllabus.md).

**Async in Python and JavaScript is the same model with one sharp difference.** Both suspend a frame and resume it later. But calling an async function in JavaScript runs the body up to the first `await` immediately, while calling one in Python executes **nothing at all** and returns a coroutine object waiting to be driven. Lives across [`02_concurrency/04`](02_concurrency/00_syllabus.md) and [`04_javascript/03`](04_javascript/00_syllabus.md).

**Speculative optimisation arrived twice.** V8's hidden classes and inline caches, and CPython 3.11's specialising adaptive interpreter, are the same idea: observe the types that actually occur, rewrite the hot path to assume them, and guard for when the assumption breaks. Lives across [`04_javascript/02`](04_javascript/00_syllabus.md) and [`01_python/05`](01_python/00_syllabus.md).

---

## 4. Coverage appendix — what the video syllabi contain and what this repo does with it

The `assets/` folder held four phone screenshots of YouTube chapter lists, treated for a while as this folder's syllabus. They are transcribed in [`_archive/2026-08_v1/video_syllabi.md`](_archive/2026-08_v1/video_syllabi.md) and the images are deleted.
