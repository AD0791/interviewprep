...
**Prerequisites:** [`03_sql/04`](../03_sql/00_syllabus.md) for module 04 — window functions transfer directly; [`06_mongodb/05`](../06_mongodb/00_syllabus.md) for module 05, which continues the same pipeline from the other side
...
```mermaid
graph LR
    SQL4["03_sql/04 window functions"] --> M4["04 analytical SQL at scale"]
    SQL7["03_sql/07 performance & ORM"] --> M1["01 execution model & slots"]
    M1 --> M2["02 partitioning & cost"]
    M2 --> M3["03 loading & ingestion"]
    M1 --> M4
    M2 --> M4
    MDB5["06_mongodb/05 pipeline out"] --> M5["05 Beam & Dataflow"]
    M3 --> M5
    CONC30["02_concurrency/07 idempotency"] --> M5
```
