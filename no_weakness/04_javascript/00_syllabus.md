...
**Feeds:** [`05_typescript/`](../05_typescript/00_syllabus.md) — all five TypeScript modules assume modules 01, 02 and 04 here
...
graph LR
    ...
    CONC4["02_concurrency/04 asyncio"] -.same model.-> M3
    M2 --> M4["04 types & coercion"]
    M3 --> M6["06 errors & leaks"]
    M1 --> M6
    M4 --> M5["05 modules & bundling"]
    M2 --> TS1["05_typescript/01 structural typing"]
    M4 --> TS1
    M3 --> CONC6["02_concurrency/06 Node & browser"]
```
...
| 03 | [`03_the_event_loop_microtasks_and_async.md`](03_the_event_loop_microtasks_and_async.md) | Stack, task queue, microtask queue; browser render steps versus Node phases; `process.nextTick` priority; the promise state machine; `async`/`await` desugared; unhandled rejections; `await` in a loop versus `Promise.all`; `AbortController`. *Diagram: interleave for a mixed sync/micro/macro program* | ~7,500 | `JS-12`–`JS-17` | ✅ **written** | `measured` — 5 IDs (`JS-LOOP-*`) |
...
Module 03 is the single Phase 4 core module, chosen because the event loop is the most-asked JavaScript topic at senior level and because it completes the cross-language concurrency story started in `02_concurrency/`.
