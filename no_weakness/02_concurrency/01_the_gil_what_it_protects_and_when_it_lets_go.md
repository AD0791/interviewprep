...
**Level:** L4–L5 · **Prerequisites:** [`01_python/05` bytecode](../01_python/05_bytecode_and_the_runtime.md)
**Syllabus:** [`CONC-01`–`CONC-05`](00_syllabus.md) · **Roles:** DE ● FS ●
...
The canonical race is two threads incrementing a shared counter. [Module 05](../01_python/05_bytecode_and_the_runtime.md) established that `balance += 1` is four separate instructions. The textbook conclusion is that a thread switch can land between any two of them, so updates get lost.
...
The fix is `ProcessPoolExecutor`, which measured 3.47×, and the cost is real: arguments and return values must be picklable, process startup is not free, and on macOS the `spawn` start method re-imports your module. Those costs are the subject of [module 03](00_syllabus.md).
...
| **`asyncio`** | Many concurrent I/O operations, thousands not dozens | One thread, far cheaper per task than a thread | Whole call stack must be async; one blocking call stalls everything ([module 04](00_syllabus.md)) |
