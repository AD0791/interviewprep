...
**Prerequisites:** [`03_sql/01`](../03_sql/00_syllabus.md) for module 02 — the index discussion is far shorter when B-trees are already understood; [`03_sql/06`](../03_sql/00_syllabus.md) for the modelling contrast in module 01
**Feeds:** [`07_bigquery/05`](../07_bigquery/00_syllabus.md) directly — module 05 ends where the BigQuery module begins
...
```mermaid
graph LR
    SQL1["03_sql/01 indexes"] --> M2["02 indexes & explain"]
    SQL6["03_sql/06 schema design"] --> M1["01 modelling & storage"]
    M1 --> M2
    M1 --> M3["03 aggregation pipeline"]
    M2 --> M3
    M1 --> M4["04 replication & durability"]
    CONC28["02_concurrency/07 MVCC"] --> M4
    M1 --> M5["05 sharding & the pipeline out"]
    M4 --> M5
    M5 --> BQ5["07_bigquery/05 Beam & Dataflow"]
```
