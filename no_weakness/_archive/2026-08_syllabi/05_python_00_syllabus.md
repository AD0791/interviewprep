...
**Feeds:** [`06_concurrency/`](06_concurrency_00_syllabus.md) (all modules), [`08_typescript/01`](08_typescript_00_syllabus.md) by analogy, [`12_bigquery/05`](12_bigquery_00_syllabus.md)
...
graph LR
    ...
    M1 --> M4["04 memory & GC"]
    M5["05 bytecode"] --> M4
    M5 --> CONC["06_concurrency/02 races"]
    M4 --> CONC3["06_concurrency/03 processes"]
    M2 --> M7["07 generators"]
    M7 --> CONC4["06_concurrency/04 asyncio"]
    M8["08 imports"] --> CONC3
```
...
|---|---|---|---|---|---|---|
| 01 | [`01_object_model_and_attribute_lookup.md`](../../05_python/01_object_model_and_attribute_lookup.md) | The full read *and* write resolution order, the descriptor protocol behind `@property` and behind every method, `__slots__` costs measured, C3 linearisation and cooperative `super()`, `__set_name__`/`__init_subclass__` versus metaclasses | 6,400 | `PY-01`–`PY-05` | ✅ **written** | `measured` — 18 IDs (`PY-OBJ-01`–`18`) |
...
| 05 | [`05_bytecode_and_the_runtime.md`](../../05_python/05_bytecode_and_the_runtime.md) | `dis`, frame objects, the eval loop, `LOAD_FAST` versus `LOAD_GLOBAL`, the 3.11+ specialising adaptive interpreter, `balance += 1` as multiple bytecodes, interning identity traps, honest `timeit` methodology | ~7,000 | `PY-23`–`PY-27` | ✅ **written** | `measured` — 8 IDs (`PY-BYT-*`) |
