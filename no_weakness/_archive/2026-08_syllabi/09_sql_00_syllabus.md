...
**Prerequisites:** none for modules 01–05; module 07 assumes [`05_python/06`](05_python_00_syllabus.md) for the ORM half
**Feeds:** [`10_mongodb/`](10_mongodb_00_syllabus.md) by contrast, [`12_bigquery/04`](12_bigquery_00_syllabus.md) directly
...
graph LR
    ...
    CONC2["06_concurrency/02 races"] --> M3
    M3 --> M6["06 schema & migrations"]
    M5 --> M6
    M4 --> BQ["12_bigquery/04"]
    M6 --> MONGO["10_mongodb/01 modelling"]
```
...
|---|---|---|---|---|---|---|
| 01 | [`01_indexes_and_the_query_planner.md`](../../09_sql/01_indexes_and_the_query_planner.md) | B-tree structure and descent, `EXPLAIN (ANALYZE, BUFFERS)`, cardinality and selectivity, leftmost prefix and skip scan, covering and index-only scans, expression and partial indexes, when statistics lie. Postgres and SQLite side by side. *Diagram: B-tree descent for a range predicate* | ~8,000 | `SQL-01`–`SQL-07` | ✅ **written** | `measured` — 7 IDs (`SQL-IDX-*`) |
